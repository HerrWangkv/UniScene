...
from pcdet.datasets.nuscenes_occ.occ_lidar_gen import load_occ_gt, cartesian_to_spherical
...
def prepare_occ(occ_file):
    ...
    data_dict['occ'] = occ
    occ_range_mask = (data_dict['occ'][:, 0] >= 0) & (data_dict['occ'][:, 0] < OCC_SIZE[0]) & \
                    (data_dict['occ'][:, 1] >= 0) & (data_dict['occ'][:, 1] < OCC_SIZE[1]) & \
                    (data_dict['occ'][:, 2] >= 0) & (data_dict['occ'][:, 2] < OCC_SIZE[2])
