...
def main(nusc, indice, nuscenesyaml, args, config):
    ...
    while int(i) < 10000:  # Assuming the sequence does not have more than 10000 frames
        ...
        mask = (np.abs(scene_points[:, 0]) < 50) & (np.abs(scene_points[:, 1]) < 50) \
        ...
        mask = (np.abs(scene_semantic_points[:, 0]) < 50) & (np.abs(scene_semantic_points[:, 1]) < 50) \
...
if __name__ == '__main__':
    ...
    parse.add_argument('--config_path', type=str, default='./data_process/config.yaml')
